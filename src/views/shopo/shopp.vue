    ...
    <el-table :data="list" style="width: 100%">
      ...
      <el-table-column label="状态" width="180">
        <template slot-scope="scope">
          ...
          <button @click="onchange(scope.row)">
...
export default {
  methods: {
    ...
    onchange(item) {
      // console.log(i);
      // this.$API.shangjia(i.id, i.status).then((res) => {
      //   console.log(res);
      ...
      this.$confirm(`确认要${a == 1 ? "上架" : "下架"}该商品？`, "提示", {
        ...
        .then(() => {
          this.$API.shangjia(item.id, a).then((res) => {
            // console.log(res);
            if (res.status == 0) {
              this.$message(res.data);
              this.$API.SHOPP(this.page).then((res) => {
                // console.log(res);
                this.list = res.data;
              });
            }
    ...
    // 查看
    detail(id) {
      this.$router.push({ path: "/shopchakan", query: { id } });
    },
    // 编辑
    set(id) {
